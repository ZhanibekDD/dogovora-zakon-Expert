...
SCHEMA_MARKER = "ZakonExpert contract schema v7 real-logo"
NAVY = "20364F"
GOLD = "B78A43"
PALE_BLUE = "F3F6F9"
PALE_GOLD = "FBF8F1"
...
# The real ZakonExpert brand logo, as a real file asset (not embedded base64 - a previous
# inline copy got truncated when pasted into source and silently corrupted every render).
LOGO_ASSET_PATH = Path(__file__).resolve().parent.parent / "templates" / "assets" / "brand" / "zakonexpert-logo.png"


class BrandAssetMissingError(RuntimeError):
    """Raised when the official logo asset is missing or unreadable.

    Deliberately fatal: a contract must never go out with a placeholder/temporary logo
    silently substituted in its place. If this fires, restore LOGO_ASSET_PATH before any
    document can be generated again.
    """


def _load_official_logo() -> bytes:
    try:
        data = LOGO_ASSET_PATH.read_bytes()
        with Image.open(io.BytesIO(data)) as probe:
            probe.load()
    except (OSError, FileNotFoundError) as exc:
        raise BrandAssetMissingError(
            f"Official logo asset missing or unreadable at {LOGO_ASSET_PATH}"
        ) from exc
    return data
...
def _replace_header_branding(doc) -> None:
    # Real logo or nothing: raises BrandAssetMissingError rather than falling back to any
    # placeholder if the asset can't be loaded - see _load_official_logo.
    logo_bytes = _load_official_logo()
